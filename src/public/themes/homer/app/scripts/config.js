...
function configBlocks($compileProvider, $httpProvider, $urlRouterProvider,
    ...
    jwtOptionsProvider.config({
        tokenGetter: ["options", "$http", "jwtHelper", function(options, $http, jwtHelper) {
            ...
            var token = Lockr.get(CFG.session.name + "_jwt");
            ...
            if (void 0 !== token && void 0 !== jwtHelper && jwtHelper.isTokenExpired(token)) {
                ...
                .then(function(response) {
                    if (response.headers("authorization")) {
                        ...
                        Lockr.set(CFG.session.name + "_jwt", token);
        ...
        whiteListedDomains: [String(CFG.session.cookie_domain), "localhost", "127.0.0.1", "0.0.0.0"]
    });
    ...
    $urlRouterProvider.otherwise(function($injector) {
        $injector.get("$state").go(
            Lockr.get(CFG.session.name + "_jwt") ? (CFG.app.default_route || "setting.index") : "login",
            {}, { reload: true });
    });
}
