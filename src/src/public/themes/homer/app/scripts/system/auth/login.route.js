...
function configBlocks($stateProvider) {
    $stateProvider
        .state("login", {
            ...
            onEnter: function() {
                switch ((CFG.auth || {}).default_driver) {
                    case "oauth2":
                        var url = CFG.auth.drivers.oauth2.redirectUri,
                            token = Lockr.get(CFG.session.name + "_ret");
                    ...
                    default:
                        Lockr.rm(CFG.session.name + "_jwt");
                }
            },
            data: {
                allowGuest: true,
        ...
        .state("oauth2", {
            ...
            onEnter: function($state) {
                ...
                if (parts[0]) {
                    Lockr.set(CFG.session.name + "_jwt", parts[0]);
                    Lockr.set(CFG.session.name + "_ret", parts[1]);

                    return $state.go(CFG.app.default_route, {}, { reload: true });
                }
